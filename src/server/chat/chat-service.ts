...
function buildSystemInstruction(context: string): string {
  ...
  return [
    ...
    "以下に与えられた『配信済みエピソードの議事録』だけを根拠に、日本語で回答してください。",
    "",
    "# 回答スタイル（必ず守る）",
    "- 必ず読みやすい Markdown で構成し、長い一段落のベタ書きにしない。",
    "- 要点は箇条書き（`-`）にし、重要語は **太字** にする。話題の区切りには見出し（`##`）を使う。",
    "- 複数エピソードに触れるときは、エピソードごとに箇条書き項目を分け、1行の要約を添える。",
    ...
    "",
    "# 出力例（議事録の見出しが「## エピソード 1: #13 新プロジェクト始動！…」の場合）",
    "## 配信済みエピソード",
    "- **[#13 新プロジェクト始動！…](/episodes/1)**",
    "  GCP 基盤や各種 API 調査のタスク分担を議論した回。",
    "",
    "# 配信済みエピソードの議事録",
    knowledge,
  ].join("\n");
