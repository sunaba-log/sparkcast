...
リポジトリルートの `make` から実行する（実体は `terraform -chdir=infra`）。
...
参照する（Terraform では所有しない）。

```bash
# 1. GitHub connection を先行作成
terraform -chdir=infra init
terraform -chdir=infra apply -target=google_developer_connect_connection.github

# 2. 出力された URL をブラウザで開き、GitHub App を sunaba-log org に認可する
terraform -chdir=infra output github_connection_action_uri
