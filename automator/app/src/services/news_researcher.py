...
上記テーマに関連する **直近1週間のニュース・技術動向から {max_items} 件**を選び、
「次の収録で話したくなる話題」として提示してください。

選定基準：
- そのテーマを先週話していた人が「それは知らなかった」「話したい」となるもの
- 「今週のAIニュースまとめ」にあるような汎用的な話題は避ける
- テーマとの具体的な接点・逆説・「そうとも言えるが…」という引っかかりがあるもの

前置き・挨拶・まとめ文は不要。以下の形式のみで出力してください：

1. **ニュースタイトル**（出典: メディア名またはURL）
・最近の会話との接続: （1文。テーマ名を含める）
・何が面白いか: （1文。意外性・逆説・論点など）
・次に話せそうな問い: （1文）
