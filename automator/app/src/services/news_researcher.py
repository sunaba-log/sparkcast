...
def _build_research_prompt(
    ...
    themes_text = "\n".join(
        f"- {t.display_name}"
        f"（直近 {t.episode_count} 回の収録で繰り返し議論、"
        f"キーワード: {' / '.join(t.keywords[:4])}）"
        for t in themes[:5]
    )

    return f"""あなたはポッドキャスト「サンドバログ」のリサーチアシスタントです。

## 私たちのポッドキャストで最近よく議論しているテーマ

{themes_text}

## タスク

上記テーマに関連する **直近1週間の注目ニュース・技術動向** を調査してください。
合計 {max_items} 件程度を選び、以下の形式で簡潔に出力してください。

各ニュース:
**[タイトル]**（関連テーマ名）
概要: 2文以内で説明
会話の種: このニュースを起点に話し合える問いを1文で

最後に「💬 今週の会話テーマ提案」を1文で締めてください。

出力は合計 400〜600 文字程度に収めてください。
"""
